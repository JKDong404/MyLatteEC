...
import com.mymxhbyf.dongjk.lattecore.ui.recycler.RgbValue;
...
public class TranslucentBehavior extends CoordinatorLayout.Behavior<Toolbar>{
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull Toolbar child, @NonNull View target, int dx, int dy, @NonNull int[] consumed, int type) {
        ...
        mDistanceY +=dy;
        //toolBar的高度
        final int targetHegiht = child.getBottom();

        //当滑动时，并且距离小于toolbar高度的时候，调整渐变色
        if (mDistanceY > 0 && mDistanceY <= targetHegiht){
            final float scale = (float) mDistanceY / targetHegiht;
            final float alpha = scale * 255;
            child.setBackgroundColor(Color.argb((int) alpha,RGB_VALUE.red(),RGB_VALUE.green(),RGB_VALUE.blue()));
        }else if (mDistanceY > targetHegiht){
